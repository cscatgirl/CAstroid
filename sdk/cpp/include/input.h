...
/**
 * Spinner state for a player
 */
struct SpinnerInput {
    mutable int step_delta = 0;   // Accumulated steps (reset after read)
    int step_resolution = 64;     // Steps per full rotation
    float angle = 0.0f;           // Current angle in radians [-π, π]

    /**
     * Get accumulated step delta and reset to 0
     * This is the recommended polling pattern - call once per frame
     */
    int getStepDelta() const {
        int delta = step_delta;
        step_delta = 0;
        return delta;
    }
